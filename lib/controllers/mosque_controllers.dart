...
class MosqueController extends GetxController {
  ...
  Future<void> fetchMosques() async {
    try {
      ...
      mosques = querySnapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
  ...
  List<Map<String, dynamic>> get filteredMosques {
    if (searchQuery.isEmpty) {
      ...
      return mosques
          .where((mosque) =>
      mosque['mosque'].toLowerCase().contains(searchQuery.value.toLowerCase()) ||
          mosque['location'].toLowerCase().contains(searchQuery.value.toLowerCase()))
          .toList();
    }
  }
